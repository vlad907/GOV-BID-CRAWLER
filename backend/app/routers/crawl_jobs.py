...
class BulkFindSuppliersRequest(BaseModel):
    source: Optional[str] = None
    is_sdvosb: Optional[bool] = None
    active_only: bool = True
    only_missing: bool = True  # skip solicitations that already have matches
    limit: int = 50


@router.post("/find-suppliers-bulk", response_model=schemas.CrawlJobOut)
def find_suppliers_bulk(payload: BulkFindSuppliersRequest, db: Session = Depends(get_db)):
    """Fans a single supplier-lookup job out across many solicitations at
    once - one target per solicitation that has an NSN."""
    from datetime import datetime

    query = db.query(models.Solicitation).filter(models.Solicitation.nsn.isnot(None))
    if payload.source:
    ...
    solicitations = query.order_by(models.Solicitation.created_at.desc()).all()
    ...
    try:
        job_id = crawler_client.submit_job("nsn_marketplace", {"targets": targets})
    except CrawlerAgentError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    ...
    job = models.CrawlJob(
        ...
        params={"target_count": len(targets)},
        status="pending",
    )
    db.add(job)
    db.commit()
